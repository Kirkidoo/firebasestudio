        ...
        try {
            ...
            if (!useCache) {
                ...
                if (operation.status !== 'COMPLETED') {
                    throw new Error(`Shopify bulk operation failed or was cancelled. Status: ${operation.status}`);
                }
                    ...
                    <div className="mt-4 p-4 border rounded-lg bg-muted/50 max-h-60 overflow-y-auto">
                        ...
                        <ul className="space-y-2 text-sm text-muted-foreground">
                            {activityLog.map((log, index) => {
                                const isDone = activityLog.length > index + 1 || log.toLowerCase().includes('finished') || log.toLowerCase().includes('complete');
                                return (
